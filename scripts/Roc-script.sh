...
sed -i "s/(\(luciversion || ''\))/(\1) + (' \/ Built by Superman')/g" feeds/luci/modules/luci-mod-status/htdocs/luci-static/resources/view/status/include/10_system.js
...
git clone --depth 1 --single-branch https://github.com/breeze303/openwrt-podman package/podman                        #firewall4的docker容器
